...
mkdir -p $(pwd)/${sample}/${sample}_${chr}
echo "running python ${libdir}/getASEventReadCounts.py --jcn1 ${tmp_jcn_file} --jnc2 ${jnc_path_prefix}_junctions.bed --genome_reads1 ${jnc_path_prefix}_genome_reads.txt.gz
 ...
 -p $(pwd)/${sample}/${sample}_${chr}/${sample}_${chr} --txt_db1 ${txt_db1} --txt_db2 ${txt_db2} --txt_db3 ${txt_db3} --jcn_seq_len ${jcn_seq_len} --sqlite_db_dir ${sqlite_db_dir}"
